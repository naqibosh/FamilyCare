...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    try (Connection conn = DatabaseConnection.getConnection()) {
        ...
        if (rs.next()) {
            // Credentials are valid; create a session
            HttpSession session = request.getSession();
            session.setAttribute("customerId", rs.getInt("cust_id")); // Store the customerId in the session
            session.setAttribute("customerName", rs.getString("cust_username")); // Store the customerName in the session
