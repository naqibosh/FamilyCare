...
function openEditModal(bookingId, staffId) {
...
function confirmDelete(bookingId) {
    var inputText = document.getElementById("confirmText-" + bookingId).value;
    if (inputText.toLowerCase() === 'confirm') {
        window.location.href = 'booking_manageProcess.jsp?action=deleteBooking&bookingId=' + bookingId;// Redirect to delete action
    } else {
...
if (success === 'true') {
    window.onload = function () {
        ...
        let interval = setInterval(function () {
            if (countdown > 0) {
                Swal.getContent().innerHTML = `<p>Booking updated successfully! Closing in ${countdown} seconds...</p>`;
                countdown--;
            } else {
